...
def estimate_error():
    global omega_z_error
    for i in range(100):
    	omega_z_error += (read_word_2c(GYRO_ZOUT_H) / 131.0) * (np.pi / 180)
...
def publish_imu(timer_event):
    ...
    imu_msg.angular_velocity.x = gyro_x * (np.pi / 180) # Converting to rad/s
    imu_msg.angular_velocity.y = gyro_y * (np.pi / 180) # Converting to rad/s
    imu_msg.angular_velocity.z = gyro_z * (np.pi / 180) - omega_z_error # Converting to rad/s and removing error estimate
